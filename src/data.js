...
export async function loadData({ hotelsUrl, bookingsUrl, usersUrl }) {
  const [hotels, bookings, users] = await Promise.all([
    fetchJson(hotelsUrl),
    fetchJson(bookingsUrl),
    usersUrl ? fetchOptionalJson(usersUrl) : []
  ]);
  ...
  const userWarnings = users.length ? validateUserRows(users) : [];
  const hotelById = new Map(hotels.map((hotel) => [hotel.hotel_id, normalizeHotel(hotel)]));
  const normalizedUsers = users.map(normalizeUser);
  const userById = new Map(normalizedUsers.map((user) => [user.user_id, user]));
  const normalizedBookings = bookings.map(normalizeBooking);
  const derivedBookings = normalizedBookings.map((booking) => deriveBookingRow(booking, hotelById, userById));
  ...
  return {
    ...
    statuses,
    amenities,
    warnings: [...hotelWarnings, ...bookingWarnings, ...userWarnings],
    loadedAt: new Date()
  };
}
...
function validateUserRows(users) {
  ...
  users.forEach((user, index) => {
    if (!("user_id" in user) && !("id" in user)) {
      warnings.push(`user row ${index + 1} is missing user_id`);
    }
  });
  return warnings;
}
...
function normalizeUser(user) {
  ...
  return {
    ...
    name: user.name || user.full_name || user.fullName || combinedName || user.username || "",
    email: user.email || "",
    phone: user.phone || user.phone_number || user.phoneNumber || user.tel || "",
    tier: user.tier || user.member_tier || user.memberTier || ""
  };
}
...
function deriveBookingRow(booking, hotelById, userById) {
  ...
  return {
    ...
    user,
    userName: user?.name || booking.user_id,
