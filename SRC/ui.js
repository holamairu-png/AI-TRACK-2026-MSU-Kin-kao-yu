import { DEMO_FALLBACK_STORAGE_KEY, generateAnomalySpotlight, generateExecutiveBrief } from "./ai.js";
...
export function initUi({ data, metrics, audit, aiPayload }) {
  ...
  bindNavigation();
  bindTheme();
  bindDemoFallbackToggle();
  renderStatus(data);
  renderDashboard(state);
  setupBookingFilters(state);
  setupHotelFilters(state);
  bindAlertsExport(state);
  bindAi(state);
  bindDetailDialog();
  renderBookings(state);
  renderHotels(state);
  renderAiPayload(state);
}

function bindDemoFallbackToggle() {
  const toggle = document.querySelector("#demoFallbackToggle");
  toggle.checked = localStorage.getItem(DEMO_FALLBACK_STORAGE_KEY) === "true";
  toggle.addEventListener("change", () => {
    localStorage.setItem(DEMO_FALLBACK_STORAGE_KEY, String(toggle.checked));
    showBanner(toggle.checked ? "Demo-safe AI enabled: using local fallback." : "Demo-safe AI disabled: using /api/ai-brief when available.", "success");
  });
}

function bindAlertsExport(state) {
  document.querySelector("#copyAlertsButton").addEventListener("click", async () => {
    const json = JSON.stringify(state.audit.response, null, 2);
    try {
      await navigator.clipboard.writeText(json);
      showBanner("Copied /ops/alerts JSON to clipboard.", "success");
    } catch (error) {
      showBanner("Clipboard blocked. Select the AI payload preview or use browser copy manually.", "warning");
...
function renderStatus(data) {
  ...
  freshness.textContent = `Loaded ${data.hotels.length} hotels and ${data.bookings.length} bookings from JSON.`;
...
function renderDashboard(state) {
  ...
  renderBarChart("#monthlyRevenueChart", metrics.revenueByMonth, "month", "revenue");
  renderStatusChart(metrics.statusDistribution);
  renderBarChart("#locationChart", metrics.topLocations, "location", "revenue");
...
function renderBarChart(selector, rows, labelKey, valueKey) {
  const max = Math.max(...rows.map((row) => row[valueKey]), 1);
  const html = rows.map((row) => {
    const width = Math.max((row[valueKey] / max) * 100, 4);
    return `
      <div class="bar-row">
  ...
  document.querySelector(selector).innerHTML = html || `<p class="empty-state">No chart data</p>`;
}
...
function setupHotelFilters(state) {
  ...
  fillSelect("#amenityFilter", ["ALL", ...state.data.amenities]);
  ...
  bindFilter("#amenityFilter", "change", (value) => state.hotelFilters.amenity = value);
  ...
  document.querySelector("#clearHotelFilters").addEventListener("click", () => {
    ...
    document.querySelector("#amenityFilter").value = "ALL";
    document.querySelector("#hotelSort").value = "revenue_desc";
    renderHotels(state);
  });

  ["#hotelSearch", "#hotelLocationFilter", "#hotelRatingFilter", "#hotelPriceFilter", "#amenityFilter", "#hotelSort"].forEach((selector) => {
...
function bindAi(state) {
  document.querySelector("#generateBriefButton").addEventListener("click", async () => {
    await runAiAction({
      ...
      action: () => generateExecutiveBrief({ apiKey: state.apiKey, payload: state.aiPayload })
    });
  });
  document.querySelector("#generateSpotlightButton").addEventListener("click", async () => {
    await runAiAction({
      ...
      action: () => generateAnomalySpotlight({ apiKey: state.apiKey, payload: state.aiPayload })
    });
  });
}
...
function renderAiPayload(state) {
  document.querySelector("#aiPayloadPreview").textContent = JSON.stringify(state.aiPayload, null, 2);
}
